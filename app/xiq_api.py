...
logger = logging.getLogger('AssignNetworkPolicy.xiq_api')
...
class XIQ:
    ...
    def __setup_post_api_call(self, info, url, payload, status_only=False):
        success = 0
        for count in range(1, self.totalretries):
            try:
                response = self.__post_api_call(url=url, payload=payload, status_only=status_only)
            except ValueError as e:
                print(f"API to {info} failed attempt {count} of {self.totalretries} with {e}")
            except Exception as e:
                print(f"API to {info} failed with {e}")
    ...
    def __post_api_call(self, url, payload, status_only=False):
        try:
    ...
    ## networkPolicy
    def collectNetworkPolicies(self, pageSize):
        ...
        nps = []
        while page <= pageCount:
            url = self.URL + "/network-policies?page=" + str(page) + "&limit=" + str(pageSize)
            rawList = self.__setup_get_api_call(info,url)
            nps = nps + rawList['data']

            if firstCall == True:
                pageCount = rawList['total_pages']
            print(f"completed page {page} of {rawList['total_pages']} collecting Network Policies")
            page = rawList['page'] + 1 
        return nps
    
    def changeNetworkPolicy(self, payload):
        info = "updating network policy on device"
        url = self.URL + "/devices/network-policy/:assign"
        response = self.__setup_post_api_call(info=info,url=url,payload=payload,status_only=True )
        return response
